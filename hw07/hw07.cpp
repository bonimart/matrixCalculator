...
{
  public:
    CIndex(const T_ & seq, C_ comp = less<typename T_::value_type>{})
      : sequence(seq.begin(), seq.end()
      ){
        comparator = comp;
    }
    set<size_t> search(const T_ & subsequence);
  
  private:
    vector<typename T_::value_type> sequence;
    function<bool(typename T_::value_type,typename T_::value_type)> comparator;

    bool compEqual(const typename T_::value_type & t1, const typename T_::value_type & t2) {
      return !((comparator)(t1, t2) || (comparator)(t2, t1));
    }

    //search first occurence of a subsequence
    template<class ForwardIt1, class ForwardIt2>
    constexpr ForwardIt1 searchFirst(ForwardIt1 first, ForwardIt1 last,
